...
public class MentionBrowser {    
    ...
    public static Mention getFirst(Mention s, Expression expression, JexlContext jexlContext) {
        ...
        while (t != null) {
            ...
            if (_filterAgree(s, t, expression, jexlContext)) {
                break;
            }
            t = t.prev();
        }
    ...
    public static List<Mention> getAllFromSameSentence(Mention s, Expression expression, JexlContext jexlContext) {
        ...
        for (Mention tt: s.document.mentions) {
            if (tt.sentNum == s.sentNum && _filterAgree(s, tt, expression, jexlContext)) {
                found.add(tt);
            }
        }
        return found;
    ...
    public static Mention getFirstFromSintax(Mention s, Expression expression, JexlContext jexlContext) {
        ...
        Mention prev = s.prev();
        while (prev != null) {
            if (s.type == Dictionaries.MentionType.NOMINAL && Filter.sentenceDistance(s, prev) > Constants.SENTENCE_WINDOW) {t = null; break;}
            if (s.type == Dictionaries.MentionType.PRONOMINAL && Filter.sentenceDistance(s, prev) > 3) {t = null; break;}
            if (_filterAgree(s, prev, expression, jexlContext)) {
